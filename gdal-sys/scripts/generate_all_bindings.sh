...
# Run from host machine (outside a container). Each GDAL version uses its corresponding container image.
#
# Usage:
...
#
# GDAL container images and their Ubuntu bases:
...
generate_bindings() {
    ...
    local image="ghcr.io/osgeo/gdal:ubuntu-small-$docker_tag"
    ...
    "$CONTAINER_ENGINE" run --platform linux/amd64 --rm \
        -v "$GDAL_SYS_DIR:/gdal-sys" \
        -e GDAL_VERSION="$version_folder" \
        "$image" \
        /gdal-sys/scripts/generate_bindings.sh
