...
use gdal::{DriverCommon, Driver, Defn, Feature, FieldDefn, FieldValue, Geometry, OGRFieldType, VectorDatasetCommon, VectorLayerCommon};
use std::fs;
use std::path::Path;
...
fn example_1() -> Result<(), Error> {
    ...
    let mut ds = drv.create_vector_only(Path::new("/tmp/output1.geojson"))?;
...
fn example_2() -> Result<(), Error> {
    ...
    let mut ds = driver.create_vector_only(Path::new("/tmp/output2.geojson"))?;
