...
use gdal_sys::{GDALDimensionGetName,GDALDimensionGetSize, CPLErr, GDALDimensionH, GDALDimensionHS, GDALGroupGetMDArrayNames, GDALGroupGetName, GDALGroupH, GDALGroupOpenMDArray, GDALMDArrayGetDataType, GDALMDArrayGetDimensionCount, GDALMDArrayGetDimensions, GDALMDArrayGetTotalElementsCount, GDALMDArrayHS};
use libc::c_void;
...
#[cfg(test)]
mod tests {
    use crate::{Dataset, DatasetOptions, GdalOpenFlags, cpl};
...
impl<'a> Dimension<'a> {
    ...
    pub fn from_c_dimension(md_array: &'a MDArray<'a>, c_dimension: *mut GDALDimensionHS) -> Self {
        Dimension { c_dimension: (c_dimension), md_array: (md_array)  }
    } 
    ...
    pub fn get_dimensions(&self) -> Result<Vec<Dimension>>
     {
        ...
        unsafe {
            ...
            println!("len {:?}", dimensions.len());

            let size = GDALDimensionGetSize(dimensions[0].c_dimension);
...
impl<'a> Group<'a> {
    ...
    pub fn md_array(&self, name: String, options: CslStringList) -> MDArray{
        ...
        let c_mdarray = unsafe {GDALGroupOpenMDArray(self.c_group, name.as_ptr(), options.as_ptr() )};
